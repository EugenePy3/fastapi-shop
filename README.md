...
│   ├── database.py              # SQLAlchemy, подключение к PostgreSQL, Base, Session
│   ├── dependencies.py          # FastAPI зависимости и авторизация
│   │
│   ├── enums/                   # Перечисления домена (OrderStatus и др.)
│   │
│   ├── mappers/                 # Преобразование ORM-моделей в DTO (Pydantic)
│   │   ├── cart_mapper.py
│   │   └── order_mapper.py
│   │
...
│   │   └── order_service.py
│   │
│   ├── schemas/                 # DTO-схемы запросов и ответов API
│   │   ├── types.py             # Общие типы (PositivePrice, Money)
│   │   ├── user.py
│   │   ├── product.py
│   │   ├── category.py
│   │   ├── cart.py
...
│       ├── cookies.py
│       └── session_utils.py
tests                            # Интеграциооное тестирование
├── api
│   ├── auth_api.py
│   ├── cart_api.py
│   ├── categories_api.py
...
| GET    | `/api/products`                       | Список товаров                    | All    |
...
| GET    | `/api/orders/{order_id}`              | Получить заказ по ID              | User   |
| PATCH  | `/api/orders/{order_id}/status`       | Изменить статус заказа            | Admin  |  
...
* **Mapper Layer** — отдельный слой преобразования ORM-моделей в Pydantic-схемы ответов (`mappers/`). Позволяет собирать вычисляемые поля (например, `subtotal`, `total` в корзине) без загрязнения моделей или схем лишней логикой.
...
Проект покрыт интеграционными тестами на **pytest**, которые проверяют работу API через реальные HTTP-запросы с использованием тестовой базы данных PostgreSQL.

Во время выполнения тестов автоматически:

* применяется актуальная схема базы данных через **Alembic**;
* база данных очищается перед каждым тестом;
* создаются отдельные тестовые пользователи и администраторы;
* проверяются бизнес-сценарии и права доступа.

### Покрытие тестами

```text
tests/
├── test_auth.py         # Регистрация, авторизация, завершение сессии
├── test_cart.py         # Управление корзиной
├── test_categories.py   # CRUD категорий
├── test_env.py          # Проверка тестового окружения
├── test_order.py        # Создание заказов, получение заказов,
│                        # изменение статуса, изоляция пользователей
└── test_products.py     # CRUD товаров и валидация данных
```

На данный момент реализовано **47 интеграционных тестов**, покрывающих:
